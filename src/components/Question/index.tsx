...
const Question: React.FC<QuestionProps> = ({ message, userName, upVotes, roomId, id, userId, answering, answered }) => {
  const { user } = useAuth()

  const updateMessage = async (data: { upVotes?: number; answering?: boolean; answered?: boolean }) => {
    try {
    ...
    <QuestionWrapper answered={answered} answering={answering}>
      ...
      <div>
        ...
        <span>
          ...
          <FiThumbsUp 
            size={20} 
            onClick={() => updateMessage({ upVotes: upVotes + 1 })} 
          />
        </span>
        )}
